...
// 2b.  By defeinind these variables at the global level, they can be accessed by any function in the script, allowing them to be used across multiple functions.
...
// - `success`: This option is a callback fujnction that is executed when the AJAX request is successful.  It takes a single argument, `response`, which represents the response data from the API.  In this case the `displayMovieDetails()` function is called and passed the `response` data as an argument to display the movie details in the UI.
...
// 7e.  In summary, this code listens for the "Add to Watchlist" buton click, retrieves the relevant movie data from the `$movieDetails` element, creates a new `movie` object, and adds it to the user's watchlist array.
...
// 8c.  If there is no watchlist dat ain local storage (i.e. if the user has not added any movies to their watchlist yet), then the `|| []` syntax is used to create an empty array in case `localStorage.getItem("watchlist")` returns `null`.
